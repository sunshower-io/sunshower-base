...
public class VersionRange {
  ...
  public boolean contains(Version v) {
    return lowerBound.compareTo(v) >= 0 && v.compareTo(upperBound) <= 0;
  }
  ...
  private static VersionRange parseRange(String version) {
    ...
    return new VersionRange(
        ...
        new Version(
            closedUpperBound,
            consumeUntil(
                version, version.length() - 2, VersionRange::isWhitespaceOrSeparator, false)));
  }
  ...
  private static String consumeUntil(
      String version, int beginIndex, Predicate<Character> p, boolean direction) {
