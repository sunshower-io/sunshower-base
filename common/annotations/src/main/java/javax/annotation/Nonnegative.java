...
public @interface Nonnegative {
  ...
  class Checker implements TypeQualifierValidator<Nonnegative> {
    ...
    public When forConstantValue(Nonnegative annotation, Object v) {
      ...
      if (value instanceof Long) isNegative = value.longValue() < 0;
      else if (value instanceof Double) isNegative = value.doubleValue() < 0;
      else if (value instanceof Float) isNegative = value.floatValue() < 0;
