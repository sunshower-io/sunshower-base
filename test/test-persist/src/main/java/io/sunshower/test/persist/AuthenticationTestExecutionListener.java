...
public class AuthenticationTestExecutionListener extends AbstractTestExecutionListener {
    ...
    public void beforeTestClass(TestContext ctx) {
        ctx.getApplicationContext().getAutowireCapableBeanFactory()
                .autowireBean(this);
    }
    ...
    public void beforeTestMethod(TestContext testContext) throws Exception {
        ...
        Map<String, GrantedAuthority> authorities = collectRoles(
                entityManager,
                ...
                if (m.isAnnotationPresent(Authority.class)) {
                    if (m.getParameterCount() == 0) {
                        ...
                        GrantedAuthority authority =
                                (GrantedAuthority) m.invoke(testInstance);
                        final String name = authority.getAuthority();
                        authorities.put(name, authority);
                    } else {
                        throw new InvalidTestException(
                                String.format("Method '%s' on type '%s' has " +
                                        "more than zero arguments", m, current));
                    }
                }
            }
        ...
        if (!GrantedAuthority.class.isAssignableFrom(type)) {
            throw new InvalidTestException(
                    String.format("Method '%s' on type '%s' has " +
                                    ...
                                    "assignable from GrantedAuthority", m
                            , current, type));
        }
    }

    private void collectRolesOnFields(EntityManagerFactory bean, Class<?> testClass, Object testInstance, Map<String, GrantedAuthority> repository) {

    }


    private void saveMethods(Class<?> testClass, Object testInstance, Map<String, GrantedAuthority> authorities) {
        Set<Object> users = usersFromMethods(testClass, testInstance, authorities);
        saveAll(users, testClass);
    }

    private void saveFields(Class<?> testClass, Object instance, Map<String, GrantedAuthority> authorities) {
    ...
    private GrantedAuthority[] resolveAuthorities(Method f, Map<String, GrantedAuthority> authorities) {
    ...
    private GrantedAuthority findAuthority(Authority a, Map<String, GrantedAuthority> authorities) {
        ...
        final String query = String.format("select a " +
                "from %s as a where " +
                "a.authority = :authority", simpleName
        );
        final List<GrantedAuthority> result = entityManager
                .createQuery(query, type)
                .setParameter("authority", a.value())
                .getResultList();
        if (result.isEmpty()) {
            throw new NoSuchElementException(String.format("No authority named '%s' " +
                    "found--did you created it with an @Authority " +
                    "annotation somewhere in your test context?", a.value()));
        }
        return result.get(0);
    }
